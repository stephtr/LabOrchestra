...
public class OscilloscopeWithStreaming : DeviceHandlerBase<OscilloscopeState>, IOscilloscope
{
    ...
    {
        _fftWindowFunction = new float[_state.FFTLength + 2];
                    ...
                    for (var i = 0; i < 500_000; i += length)
                    {
                        ...
                        for (var ch = 0; ch < 4; ch++)
                        {
                            ...
                            if (_state.Channels[ch].ChannelActive && _buffer[ch].Count > length)
                            {
                                ...
                                {
                                    fftNp = fftNp * np.array(_fftWindowFunction);
                                }
                                ...
                                storage = storage * oldWeight + val * newWeight;
                                _fftStorage[ch] = storage.ToArray<double>();
                                _acquiredFFTs[ch]++;
                            }
                        }
